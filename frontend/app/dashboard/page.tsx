...
// creating custom hook to load zaps
function useZaps(){
    const [loading,setLoading] = useState(true);
    const [zaps,setZaps] = useState<Zap[]>([]);
...
export default function(){
    ...
    return <div>
        ...
        {/* change logic inside AppBar in order to render buttons conditionally  */}
        <AppBar/>
        <div className="flex justify-center pt-8">
                <div className="max-w-screen-lg w-full">
                    <div className="flex justify-between pr-5">
                        <div className="text-2xl font-strong">
                            My Zaps 
                        </div>
                        <DarkButton onClick={()=>{
                            router.push("/zap/create");
        ...
        {/* return a table of zaps if not loading */}
        {loading?"Loading...":<div className="flex jusitfy-center w-full"><ZapTable zaps={zaps}/></div>}
    </div>
}
...
function ZapTable({ zaps } : {zaps:Zap[]}){
    ...
    <div className="flex">
        ...
        {zaps.map(z=> <div className="flex border-b border-t py-4 ">
            ...
            <div className="flex-1">{z.id}</div>
            <div className="flex-1">Some date</div>
            <div className="flex-1"><LinkButton onClick={()=>{
                router.push("/zap/"+z.id)
            }} >Go</LinkButton></div>
